...
public class ReviewSpecification {
    ...
    public static Specification<Review> hasTransportOrganizerId(Integer transportOrganizerId) {
        return (Root<Review> root, CriteriaQuery<?> query, CriteriaBuilder cb) -> {
            if (transportOrganizerId == null) {
                return cb.conjunction();
            }
            ...
            Join<Ticket, Trip> tripJoin = ticketJoin.join("trip");
            Join<Trip, Route> routeJoin = tripJoin.join("route");
            Join<Route, TransportOrganizer> tranOrgJoin = routeJoin.join("tranOrg");

            return cb.equal(tranOrgJoin.get("tranOrgId"), transportOrganizerId);
        };
    }
